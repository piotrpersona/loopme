...
const LOCAL_STORAGE_SAVE_KEY = "loopme.records"
...
export default function Looper() {
    ...
    var saveRecord = () => {
        ...
        var records = getSavedRecords()
        records[hash(data)] = data
        saveRecords(records)
        return records
    }

    var saveRecords = (records) => {
        localStorage.setItem(LOCAL_STORAGE_SAVE_KEY, JSON.stringify(records));
    }

    var displaySaveDialog = (e) => {
        setDisplaySavePane(true)
    }

    var browse = (e) => {
        var savedRecords = getSavedRecords()
        Object.keys(savedRecords).forEach(key => {
            var record = savedRecords[key]
            record['removeFn'] = removeSingleRecord(key)
        })
        setSavedRecords(savedRecords)
        setDisplaySavePane(true)
    }

    var handleSaveRecordName = (e) => {
        setRecordName(e.target.value)
    }

    var handleSaveRecord = (e) => {
        saveRecord()
    }

    var removeSingleRecord = (id) => {
        return (e) => {
            var records = getSavedRecords()
            delete records[id]
            saveRecords(records)
